...
def get_file_extension(file_path_or_url):
    ...
    if os.path.exists(file_path_or_url):  # Local file check
        return os.path.splitext(file_path_or_url)[1].lower().lstrip(".")
...
def detect_heic(file_path):
    """Detect if a file is HEIC format, even if mislabeled."""
    try:
...
def convert_heic_to_jpg(heic_path):
    ...
    try:
        ...
        logging.error(f"❌ HEIC Conversion failed: {heic_path} | {e}")
        return None
...
def process_images():
    ...
    for timestamp, image_url, stave_count in data:
        ...
        if downloaded_file:
            ...
            final_path = move_file_to_type_folder(downloaded_file)

            # Convert HEIC files to JPG and store them in JPEG folder
            if detect_heic(final_path):
                converted_path = convert_heic_to_jpg(final_path)
                if converted_path:
                    move_file_to_type_folder(converted_path)
